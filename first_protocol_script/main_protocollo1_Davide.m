...
aquisitions.tri_ct.kg_2.raw = data(9:end,:);
aquisitions.tri_ct.kg_2.raw([3 2 1 8 7 6 5 4],:) = aquisitions.tri_ct.kg_2.raw([4 5 6 7 8 1 2 3],:);

load("iso_4kg_tricipite.mat") ; 
aquisitions.tri_ct.kg_4.raw = data(9:end,:);
aquisitions.tri_ct.kg_4.raw([3 2 1 8 7 6 5 4],:) = aquisitions.tri_ct.kg_4.raw([4 5 6 7 8 1 2 3],:);

load("iso_6kg_tricipite_nosat.mat") ; % G = 500
aquisitions.tri_ct.kg_6.raw = data(9:end,:)*2;
aquisitions.tri_ct.kg_6.raw([3 2 1 8 7 6 5 4],:) = aquisitions.tri_ct.kg_6.raw([4 5 6 7 8 1 2 3],:);
...
for m = 1:N_mus
    for l = 1:N_loads
        ...
        if m == 1 % Bicipite
        canale_migliore = 5;
        aquisitions.(muscles{m}).(loads{l}).CV_cross_corr = (cv)';

        % Istruzione per inserire nelle metriche globali
        global_metrics.(muscles{m}).(loads{l}).CV.mean_value = (cv(canale_migliore,:))';

        else % Tricipite, la cv non è attendibile perchè non eravamo bene allineati alle fibre, il valore assoluto non possiamo considerarlo; 
             % al massimo, possono essere considerati gli andamenti e basta
        canale_migliore = 1;
        aquisitions.(muscles{m}).(loads{l}).CV_cross_corr = (cv)';
...
for m = 1:N_mus
    for l = 1:N_loads
        x = aquisitions.(muscles{m}).(loads{l}).mono_filt'; % Segnale monopolare
        for ch = 1:length(x(1,:)) 
...
for i = 1: length(metric_names_plot)
    subplot(1,length(metric_names_plot),i)
    for l = 1:N_loads
        T = length(aquisitions.tri.(loads{l}).mono_filt)/fs; % Durata temporale della acquisizione
        t_epo = linspace(0,T,n_epo);
...
aquisitions.bic_ct.kg_2.mono_filt = aquisitions.tri_ct.kg_2.mono_filt + aquisitions.bic.kg_2.mono_filt;
aquisitions.bic_ct.kg_4.mono_filt = aquisitions.tri_ct.kg_4.mono_filt + aquisitions.bic.kg_4.mono_filt;
aquisitions.bic_ct.kg_6.mono_filt = aquisitions.tri_ct.kg_6.mono_filt + aquisitions.bic.kg_6.mono_filt;
...
for l = 1:N_loads
    for ch=1:N_ch
    x = aquisitions.bic_ct.(loads{l}).mono_filt(ch,:); % Segnale monopolare

    miscele = [x; aquisitions.tri.(loads{l}).mono_filt(ch,:)];

    %----------PCA--------%
    [coeff,scores,var] = pca(miscele');

    %computo l'output per il segnale in  uscita con la matrice A del
    %sistema lineare : Ax = b , con b autovali del segnale che sono
    %i valori in 'var'

    output_signal = coeff * miscele;

    % faccio il for per avere in uscita il segnale normalizzato
    % sulla std del segnale stesso

    for i=1:size(output_signal,1);output_signal(i,:)=output_signal(i,:)/std(output_signal(i,:));end

    % salvo le variabili del segnale e l'output del segnale stesso
...
for l = 1:1%N_loads
        ...
        MyPlot(aquisitions.bic.(loads{l}).mono_filt(ch,:),1,['Bicipite, kg_',num],fs);
        subplot(2,2,2);
        MyPlot(aquisitions.tri_ct.(loads{l}).mono_filt(ch,:),1,['Tricipite, kg_',num],fs);   

        subplot(2,2,3);
        miscele = PCA.bic_ct.(loads{l}).(channel{ch}).miscele;
        MyPlot(miscele/range(miscele(:)),1,'Miscele',fs);     % plot mixing 1
...
%% Ricostruzione delle sorgenti e separazione 
clear RR
clear J
clear Y
...
for l = 1:N_loads
    for ch = 1:N_ch
        for ind1 = 1:2
            ...
            RR(ind1, ind2) = (sum(y.*x))/(norm(y)*norm(x));

            end
        end

        for ind = 1:2
            [~,J(ind)] = max(abs(RR(ind,:)));
        end
